...
\subsection{Fitting Problems and Stochastic Gradients}\label{sec:fitting}

The problems at the heart of machine learning have a specific additive structure worth exploiting.

\begin{definition}[Fitting problem]\label{def:fitting}
...
\begin{definition}[Incremental gradient]\label{def:incremental-grad}
...
\begin{definition}[Stochastic gradient]\label{def:stochastic-grad}
...
An inexact gradient that points roughly in the right direction can still drive convergence: a property that proves decisive when derivatives don't even exist.


\subsection{Regularization and Feature Selection}\label{sec:regularization}
...
\end{itemize}
An alternative is \emph{feature selection}: directly restricting which components of $w$ can be nonzero.
...
\subsection{Classification: SVM and SVR}\label{sec:svm-svr}
...
\begin{definition}[Support Vector Machine]\label{def:svm}
	\begin{equation}\label{eq:svm}
		\min_{w, b}\Bigl\{\norm{w}^2 + C \sum_{s \in S} \max\{1 - y^s(\inner{w, x^s} - b),\; 0\}\Bigr\}.
	\end{equation}
	The term $\norm{w}^2$ is the regularizer (model complexity), $C$ weights the empirical loss, and $\max\{\cdot, 0\}$ is the \emph{hinge loss}.
	The hinge loss makes the objective convex but non-differentiable: $[\max\{\cdot, 0\}]'(0)$ is undefined.
\end{definition}

\begin{definition}[Support Vector Regression]\label{def:svr}
...
\subsection{Why Smooth Methods Fail}\label{sec:smooth-fail}

When $f \notin C^1$, fixed-stepsize gradient descent breaks.
The direction $-g$ for $g \in \partial f(x)$ (a \emph{subgradient}, defined below) may not be a descent direction: there can exist points where $f(x + \alpha d) \geq f(x)$ for all $\alpha > 0$ and some $d = -g$.
...
\subsection{Subgradients and the Subdifferential}\label{sec:subgradients}
...
\begin{definition}[Subgradient]\label{def:subgradient}
	A vector $g \in \R^n$ is a \emph{subgradient} of a convex function $f$ at $x$ if
...
\begin{definition}[Subdifferential]\label{def:subdifferential}
	...
	The \emph{steepest descent subgradient} is $g^* = \argmin_{g \in \partial f(x)} \norm{g}$, and $0 \in \partial f(x)$ iff $x$ is a global minimum.
\end{definition}
...
\begin{example}[Subdifferential of a pointwise maximum]\label{exm:subdiff-max}
...
\begin{definition}[DSS condition]\label{def:dss}
...
\begin{definition}[Polyak stepsize]\label{def:polyak}
...
\end{equation}
Two convergence strategies: \emph{stepsize-restricted} (deflection grows $\Rightarrow$ stepsize shrinks) or \emph{deflection-restricted} (stepsize chosen first $\Rightarrow$ deflection adapts) \cite{dantonio2009}.
The deflection parameter can be chosen optimally: $\gamma^k = \argmin_{\gamma \in [0,1]} \norm{\gamma g^k + (1 - \gamma) d^{k-1}}^2$.


\subsection{Smoothed Gradient Methods}\label{sec:smoothing}
...
\begin{theorem}[Smoothing bounds]\label{thm:smoothing}
	Let $K = \max\{\norm{z}^2/2 : z \in Z\}$.
...
\subsection{Bundle Methods}\label{sec:bundle-methods}
...
\begin{definition}[Cutting plane model]\label{def:bundle}
	The \emph{bundle} at step $k$ collects oracle outputs: $\mathcal{B}^k = \{(x^j, f^j, g^j) : j < k\}$.
	The \emph{cutting plane model} is
...
\begin{theorem}[Convex functions as maxima of affine functions]\label{thm:convex-max-affine}
...
The \emph{stabilized master problem} is
\begin{equation}\label{eq:proximal-bundle}
	\min_x \Bigl\{f_{\mathcal{B}}(x) + \frac{\mu}{2}\norm{x - \bar{x}}^2\Bigr\},
\end{equation}
where $\bar{x}$ is the \emph{stability center} (best point so far) and $\mu > 0$ controls regularization.
...
The algorithm alternates between \emph{serious steps} (the candidate significantly improves $f$, so $\bar{x}$ is updated) and \emph{null steps} (improvement is insufficient, only the bundle is enriched).
