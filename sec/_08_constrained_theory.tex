...
\subsection{Problem Formulation}\label{sec:constrained-formulation}

\begin{definition}[Constrained optimization problem]\label{def:constrained-P}
...
\begin{definition}[Indicator function]\label{def:indicator}
	...
	Using it, $(P)$ becomes $\min\{f(x) + \mathbb{I}_X(x)\}$: the \emph{essential objective}.
	But $\mathbb{I}_X \notin C^0$, making the problem harder.
\end{definition}

It is sometimes useful to hide constraints in the objective: the \emph{epigraph reformulation} $(P) \equiv \min\{v : v \geq f(x),\, x \in X\}$ turns a nonlinear objective into a linear one at the cost of an extra variable and a nonlinear constraint.
If $X = \emptyset$, the problem is infeasible and $\nu(P) = +\infty$.

\begin{definition}[Interior, boundary, local optimum]\label{def:interior-boundary}
...
\begin{definition}[Open, closed, full-dimensional sets]\label{def:set-classes}
	$S$ is \emph{open} if $S = \mathrm{int}(S)$; \emph{closed} if it contains all its limit points ($x_i \to x$, $x_i \in S \Rightarrow x \in S$); \emph{full-dimensional} if $\mathrm{int}(S) \neq \emptyset$.
	Finite intersections of open sets are open; finite unions of closed sets are closed.
\end{definition}


\subsection{Tangent Cone and First-Order Geometric Conditions}\label{sec:tangent-cone}
...
\begin{definition}[Tangent cone]\label{def:tangent-cone}
	$T_X(x) = \{d \in \R^n : \exists\, \{z_i \in X\} \to x,\; \{t_i\} \to 0,\; t_i > 0 : (z_i - x)/t_i \to d\}$.
\end{definition}

\begin{definition}[Tangent cone condition (TCC)]\label{def:tcc}
	$\inner{\nabla f(x), d} \geq 0$ for all $d \in T_X(x)$.
\end{definition}

\begin{theorem}[TCC is necessary for local optimality]\label{thm:tcc-necessary}
...
If $X$ is convex, TCC is also \emph{sufficient} for global optimality (since $X \subseteq x + T_X(x)$).
...
\subsection{Convexity of Sets}\label{sec:convex-sets}
...
\begin{definition}[Convex set]\label{def:convex-set}
	...
	The \emph{convex hull} $\conv(S)$ is the smallest convex set containing $S$; $C$ is convex iff $C = \conv(C)$.
\end{definition}
...
\subsection{Feasible Directions and Algebraic Formulation}\label{sec:feasible-directions}

\begin{definition}[Feasible direction cone]\label{def:feasible-dir}
...
\begin{definition}[Algebraic form]\label{def:algebraic}
...
Linear constraints $g_i(x) = \inner{A_i, x} + b_i \leq 0$ define half-spaces; their intersection is a \emph{polyhedron}.
...
This gives the \emph{Poorman's KKT}: $x$ is optimal iff $Ax = b$ and $\exists\, \mu : \nabla f(x) = A^T \mu$ \cite{luenberger2008}.

The \emph{reduced problem} eliminates the equality constraints: partition $A = [A_B \mid A_N]$ with $A_B$ invertible, set $x_B = A_B^{-1}(b - A_N x_N)$, and optimize over $x_N$ alone.
The reduced gradient $\nabla r(x_N) = D^T \nabla f(Dx_N + d)$ recovers the KKT conditions at its zero.


\subsection{Nonlinear Constraints and KKT}\label{sec:kkt}

\begin{definition}[Active constraints and first-order feasible direction cone]\label{def:active-dx}
...
\emph{Constraint qualifications} are conditions under which $T_X(x) = D_X(x)$:
\begin{itemize}
...
\begin{definition}[Dual cone and Farkas' lemma]\label{def:farkas}
...
\begin{theorem}[Karush--Kuhn--Tucker conditions]\label{thm:kkt}
...
\subsection{Second-Order Conditions}\label{sec:sosc}

When $(P)$ is not convex, KKT is necessary but not sufficient.
Second-order conditions fill the gap.

\begin{definition}[Lagrangian and critical cone]\label{def:lagrangian}
	The \emph{Lagrangian} is $L(x; \lambda, \mu) = f(x) + \sum_i \lambda_i g_i(x) + \sum_j \mu_j h_j(x)$.
	At a KKT point, $\nabla_x L = 0$.
	The \emph{critical cone} restricts attention to directions consistent with the active constraints:
...
\begin{theorem}[Second-order conditions]\label{thm:sosc}
...
\subsection{Lagrangian Duality}\label{sec:duality}
...
\begin{definition}[Lagrangian relaxation and dual function]\label{def:lag-relax}
	$\psi(\lambda, \mu) = \min_{x} L(x; \lambda, \mu)$.
	For any $\lambda \geq 0$ and any $\mu$: $\psi(\lambda, \mu) \leq \nu(P)$ (\emph{weak duality}).
...
\begin{definition}[Lagrangian dual]\label{def:lag-dual}
	$(D) \quad \max\{\psi(\lambda, \mu) : \lambda \geq 0,\, \mu \in \R^{|J|}\}$.
	This is a convex program (even if $(P)$ is not), providing a lower bound $\nu(D) \leq \nu(P)$.
\end{definition}

\begin{definition}[Strong duality]\label{def:strong-dual}
	If $\nu(D) = \nu(P)$, we have \emph{strong duality}: the bound is tight and the dual solution recovers all the information of the primal.
\end{definition}
...
\begin{theorem}[Strong duality for convex problems]\label{thm:strong-dual-convex}
...
The optimal multiplier $\lambda_i^*$ measures the \emph{sensitivity} of $f^*$ to a relaxation of constraint $i$: loosening $g_i(x) \leq 0$ to $g_i(x) \leq \varepsilon$ changes $f^*$ by approximately $-\lambda_i^* \varepsilon$.
Active constraints with large $\lambda_i^*$ are ``expensive'': removing them would significantly improve the objective.


\subsection{Specialized Duals}\label{sec:specialized-duals}
...
\subsection{Conic Programs}\label{sec:conic}

\begin{definition}[Conic program]\label{def:conic}
...
\begin{definition}[Conic dual]\label{def:conic-dual}
...
\subsection{Proximal Bundle Method for Lagrangian Duals}\label{sec:bundle-dual}

When $(D)$ involves a nonsmooth $\psi$, the proximal bundle method from \Cref{sec:bundle-methods} applies directly.
The bundle $\mathcal{B} = \{(x^h, f^h, g^h)\}_{h < i}$ builds a cutting-plane model $f_{\mathcal{B}}^i \leq f$.

The stabilized master problem uses the \emph{linearization error} $\alpha^h = f(\bar{x}) - f^h - \inner{g^h, \bar{x} - x^h} \geq 0$.
...
\subsection{SVM and SVR: A Duality Application}\label{sec:svm-duality}

The nonsmooth SVM and SVR formulations from \Cref{sec:svm-svr} can be reformulated using slack variables and solved via their duals.
...
\end{equation}
The primal solution recovers as $w^* = \sum_i \alpha_i^* y^i x^i$; only \emph{support vectors} ($\alpha_i^* > 0$) contribute.
...
The dual depends on inner products $\inner{x^i, x^j}$ only, which we replace with a \emph{kernel function} $\kappa(x^i, x^j) = \inner{\varphi(x^i), \varphi(x^j)}$, computed without forming $\varphi$ explicitly.
