...
This section develops the key univariate methods, from slow and reliable to fast and fragile. At each step, we trade a stronger assumption on $f$ for a faster algorithm. The section ends with the question: what if the local optimum we find is not the global one?


\subsection{Lipschitz Continuity: Making Optimization Possible}\label{sec:lipschitz}

\Cref{sec:why-hard} showed that optimization is impossible for arbitrary functions: isolated minima can hide in arbitrarily narrow spikes.
...
\begin{theorem}[Lipschitz implies continuity]\label{thm:lip-cont}
...
\begin{theorem}[Complexity for Lipschitz functions]\label{thm:lip-complexity}
...
Second, a method that works well for one class of functions may fail on another: the \emph{no free lunch theorem} \cite{serafino2014} says that, averaged over all possible objectives, every algorithm performs equally badly.
...
\subsection{Local Optimality}\label{sec:local-opt}
...
\begin{definition}[Local minimum]\label{def:local-min}
...
\begin{definition}[Strictly unimodal function]\label{def:unimodal}
...
But near any local minimum $x^*$, a typical $C^1$ function \emph{is} unimodal within some neighborhood: the \emph{attraction basin} of $x^*$.
...
Finding the right basin is an entirely different (and much harder) problem, which we defer to \Cref{sec:global-opt}.
...
\subsection{Golden Ratio Search}\label{sec:golden-ratio}
...
\subsection{Derivatives and First-Order Models}\label{sec:derivatives}
...
\begin{definition}[Derivative and first-order model]\label{def:derivative}
...
Therefore $f'(x^*) = 0$ at every local minimum, a \emph{necessary} condition.
...
\subsection{Dichotomic Search}\label{sec:dichotomic}
...
A simple strategy: \emph{exponential expansion}.
...
\subsection{Quadratic Interpolation}\label{sec:quad-interpolation}
...
\begin{theorem}[Superlinear convergence of quadratic interpolation]\label{thm:secant-conv}
...
\subsection{Newton's Method}\label{sec:newton-univariate}
...
\subsection{Global Optimization}\label{sec:global-opt}
...
\begin{theorem}[Convexity and global optimality]\label{thm:convex-global}
...
When convexity doesn't hold and the global optimum is genuinely needed, the standard approach is \emph{spatial branch-and-bound}.
...
Subregions whose lower bound exceeds the best known value are \emph{pruned}: they cannot contain the global minimum.
...
\subsection{Summary: Regularity and Speed}\label{sec:univariate-summary}
