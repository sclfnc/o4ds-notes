...
Finding one is the \emph{how}, and the job of this section.
...
\subsection{Equality-Constrained Quadratic Programs}\label{sec:eq-constrained-qp}
...
The \emph{reduced Hessian} $\tilde{Q} = D^T Q D \in \R^{(n-m) \times (n-m)}$ governs the reduced problem $\min_{x_N} \frac{1}{2} x_N^T \tilde{Q}\, x_N + (D^T Q d + D^T q)^T x_N$.
...
\subsection{Active Set Methods}\label{sec:active-set}
...
\subsection{Projected Gradient Methods}\label{sec:projected-gradient}
...
\subsection{Frank--Wolfe (Conditional Gradient) Method}\label{sec:frank-wolfe}
...
\subsection{Dual Methods}\label{sec:dual-methods}
...
\subsection{Barrier Methods}\label{sec:barrier}
...
For each $\gamma > 0$, the unique minimizer $x_\gamma$ traces the \emph{central path} $\mathcal{C} = \{x_\gamma : \gamma \in (0, \infty)\}$.
As $\gamma \to 0$, $x_\gamma \to x^*$ (the constrained optimum); as $\gamma \to \infty$, $x_\gamma$ approaches the \emph{analytical center} $x_\infty = \argmax\{\prod_i (b_i - A_i x)\}$.

The \emph{path-following} strategy: start near the analytical center and decrease $\gamma$ toward zero, tracking the central path with Newton steps.
The logarithmic barrier is \emph{self-concordant}, ensuring that Newton's method behaves well.
...
\subsection{Primal-Dual Interior Point Methods}\label{sec:ipm}
...
The \emph{predictor-corrector} approach solves the Newton system twice: first a ``prediction'' (affine scaling), then a ``correction'' that adds the nonlinear term $\Delta\Lambda\Delta S\, u$, reusing the matrix factorization.
