...
\subsection{Optimization Problems}\label{sec:opt-problems}

\begin{definition}[Univariate function]\label{def:univariate-fn}
...
\begin{definition}[Optimization problem]\label{def:minimization}
	Given a function $f$ (the \emph{objective}) and a set $X \subseteq \R^n$ (the \emph{feasible region}), the optimization problem is
	...
	When $X = \R^n$, the problem is \emph{unconstrained}; otherwise it is \emph{constrained}. The optimal value $f^* = \nu(P)$ is unique, but the minimizer $x^*$ need not be. All minimizers share the same objective value, so they are equivalent ``in the eyes of $f$.'' We write $X^* = L(f, f^*) \cap X$ for the \emph{optimal set}, and call any $x \in X$ a \emph{feasible solution} and any $x \notin X$ an \emph{infeasible} one.
\end{definition}

\begin{definition}[Types of constraints]\label{def:constraint-types}
	...
	\end{itemize}
	A common special case: \emph{box constraints} $x_- \leq x \leq x_+$ with $x_-, x_+ \in \R^n$ and $x_- \leq x_+$ component-wise.
...
\subsection{Simple Reformulations}\label{sec:reformulations}

Several transformations change $f^*$ in a predictable way without affecting $X^*$.
These are \emph{reformulations}: equivalent problems with identical optimal sets.
...
\subsection{When the Optimum Does Not Exist}\label{sec:unbounded}
...
\begin{definition}[Function unbounded below]\label{def:unbounded-below}
	A function $f$ is \emph{unbounded below} if for every $M > 0$ there exists some $x$ with $f(x) < -M$. No finite minimum exists; we write $f^* = -\infty$ as shorthand for ``there is no finite lower bound on $\im(f)$.''
...
The \emph{extended reals} $\Rbar = \{-\infty\} \cup \R \cup \{+\infty\}$ resolve the second issue: every subset of $\Rbar$ has an infimum and a supremum in $\Rbar$.
We adopt the conventions $\inf \emptyset = +\infty$ and $\sup \emptyset = -\infty$.

With this, $f^*$ in \cref{eq:opt-problem} always exists in $\Rbar$.
...
\subsection{Approximate Solutions and Optimality Gaps}\label{sec:solution-quality}

\begin{definition}[Approximate solution]\label{def:approx-solution}
	Fix a tolerance $\varepsilon > 0$.
	A point $x_\varepsilon$ is an \emph{$\varepsilon$-optimal solution} of $(P)$ if
...
\subsection{Why Optimization is Hard}\label{sec:why-hard}
...
\subsection{Multivariate Problems}\label{sec:multivariate}

\begin{definition}[Multivariate optimization]\label{def:multivariate}
	When $f \colon \R^n \to \R$ with $n > 1$, the feasible region can take many forms.
	A simple but common one is the \emph{box} (hyperrectangle):
...
With $n = 30$, that is already over a billion ($2^{30} \approx 1.1 \times 10^9$); with $n = 100$, about $10^{30}$, far beyond any feasible enumeration.
...
A more general tool (the \emph{tomography}) slices the function along a chosen direction, reducing the multivariate problem to a univariate one.
We develop this idea in \Cref{sec:spectral}.


\subsection{Multi-objective Optimization}\label{sec:multi-obj}
...
\begin{definition}[Pareto optimality]\label{def:pareto}
	Since $\R^k$ for $k > 1$ lacks a total order, there is no single ``best'' solution.
	A feasible point $x^*$ is \emph{Pareto-optimal} (or \emph{non-dominated}) if no other $x \in X$ satisfies
	$f_i(x) \leq f_i(x^*)$ for all $i$ with strict inequality for at least one $i$.
	The set of all Pareto-optimal points is the \emph{Pareto frontier}.
\end{definition}
