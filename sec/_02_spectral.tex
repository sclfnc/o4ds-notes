...
\subsection{Scalar Products, Norms, and Distances}\label{sec:inner-product}

Before analyzing any function on $\R^n$, we need the geometric primitives: how to measure angles, lengths, and proximity.

\begin{definition}[Scalar product]\label{def:scalar-product}
	The \emph{(Euclidean) scalar product} of $x, z \in \R^n$ is
...
The \emph{Cauchy--Schwarz inequality} $\abs{\inner{x, z}} \leq \norm{x} \norm{z}$ is an immediate consequence.

\begin{definition}[Norm and distance]\label{def:norm-distance}
	The \emph{(Euclidean) norm} of $x \in \R^n$ is $\norm{x} = \sqrt{\inner{x, x}} = \sqrt{x_1^2 + \cdots + x_n^2}$.
	It satisfies: $\norm{x} \geq 0$ with equality iff $x = 0$; $\norm{\alpha x} = \abs{\alpha} \norm{x}$; and the \emph{triangle inequality} $\norm{x + z} \leq \norm{x} + \norm{z}$.
	Two related identities: $\norm{x + z}^2 = \norm{x}^2 + \norm{z}^2 + 2\inner{x, z}$ and the \emph{parallelogram law} $2\norm{x}^2 + 2\norm{z}^2 = \norm{x + z}^2 + \norm{x - z}^2$.
	The \emph{distance} between $x$ and $z$ is $d(x, z) = \norm{x - z}$.
\end{definition}
...
The \emph{transpose} $A^T \in \R^{n \times m}$ exchanges rows with columns: $(A^T)_{ij} = A_{ji}$.
A square matrix $A \in \R^{n \times n}$ is \emph{symmetric} if $A = A^T$, equivalently $A_{ij} = A_{ji}$ for all $i, j$.
A scalar $a \in \R$ can be viewed as a $1 \times 1$ symmetric matrix.

$\R^n$ is a finite-dimensional vector space of dimension $n$.
The \emph{canonical basis} $\{u^1, \dots, u^n\}$ consists of the $n$ unit vectors: $u^i_j = 1$ if $j = i$ and $0$ otherwise.
...
The \emph{spectral norm} (or \emph{operator norm}) of $A$ is
...
The \emph{spectral radius} $\rho(A) = \max_i |\lambda_i(A)|$ satisfies $\rho(A) \leq \norm{A}$, with equality when $A$ is symmetric.


\subsection{Linear Functions}\label{sec:linear-functions}
...
A \emph{linear univariate function} is $f(x) = bx$ for a fixed $b \in \R$.
The parameter $b$ is the \emph{slope}: if $b > 0$, $f$ is increasing; if $b < 0$, decreasing; if $b = 0$, constant.
...
A \emph{linear multivariate function} is $f(x) = \inner{b, x} = \sum_{i=1}^{n} b_i x_i$ for a fixed $b \in \R^n$.
...
\subsection{Quadratic Univariate Functions}\label{sec:quad-univariate}
...
The point $\bar{x}$ is the \emph{center} of the function: the vertex of the parabola.
If $a > 0$, the unique minimum is at $\bar{x}$; if $a < 0$, the unique maximum.
Still $\mathcal{O}(1)$.

\begin{remark}\label{rem:completing-square}
	This ``completing the square'' trick will reappear in the multivariate setting (\Cref{sec:quad-nonhom}).
...
\subsection{Separable Quadratic Functions}\label{sec:separable-quad}

A \emph{separable (non-homogeneous) quadratic} is
...
\subsection{Tomographies}\label{sec:tomographies}
...
\begin{definition}[Tomography]\label{def:tomography}
	Given $f \colon \R^n \to \R$, an origin $x \in \R^n$, and a direction $d \in \R^n$, the \emph{tomography} of $f$ along $d$ from $x$ is
...
\subsection{The General Quadratic Function}\label{sec:general-quad}
...
\subsection{Eigenvalues and Spectral Decomposition}\label{sec:spectral-decomp}

\begin{definition}[Eigenvalues and eigenvectors]\label{def:eigen}
	Given a matrix $Q \in \R^{n \times n}$, a scalar $\lambda \in \R$ is an \emph{eigenvalue} and a nonzero vector $v \in \R^n$ is the corresponding \emph{eigenvector} if $Qv = \lambda v$.
...
\begin{theorem}[Spectral theorem for symmetric matrices]\label{thm:sym-eigen}
...
\begin{definition}[Eigenvector matrix and spectral decomposition]\label{def:spectral-decomp}
	Collecting the eigenvectors into $H = [H_1 \mid \cdots \mid H_n] \in \R^{n \times n}$, orthonormality gives $H^T H = I$ (so $H$ is orthogonal).
	The \emph{spectral decomposition} is
...
\begin{theorem}[Variational characterization of eigenvalues]\label{thm:var-eigen}
...
\subsection{Matrix Definiteness}\label{sec:definiteness}
...
\begin{definition}[Matrix definiteness]\label{def:definiteness}
	A symmetric matrix $Q$ is:
	\begin{itemize}
		\item \emph{Positive definite} ($Q \succ 0$): all $\lambda_i > 0$.
			The quadratic $\frac{1}{2} x^T Q x$ has a unique global minimum at the origin.
		\item \emph{Negative definite} ($Q \prec 0$): all $\lambda_i < 0$.
			Unique global maximum at the origin.
		\item \emph{Positive semidefinite} ($Q \succeq 0$): all $\lambda_i \geq 0$.
			The quadratic is convex, but the minimum need not be unique: flat directions correspond to zero eigenvalues.
		\item \emph{Negative semidefinite} ($Q \preceq 0$): all $\lambda_i \leq 0$.
			Concave, with a possibly non-unique maximum.
		\item \emph{Indefinite}: eigenvalues of both signs.
			The origin is a saddle point.
	\end{itemize}
\end{definition}


\subsection{Tomographies of Quadratic Functions}\label{sec:quad-tomographies}
...
\begin{example}[Tomographies under different definiteness]\label{exm:quad-tomographies}
...
\subsection{Level Sets and Geometry}\label{sec:level-sets}
...
\subsection{Optimizing Quadratic Functions}\label{sec:opt-quad}

Definiteness of $Q$ and the presence of a linear term jointly determine whether (and how) a quadratic can be optimized.

\subsubsection{Homogeneous Multivariate Quadratics}\label{sec:opt-quad-hom}
...
\subsubsection{Non-homogeneous, Nonsingular Case}\label{sec:quad-nonhom}
...
\subsubsection{Non-homogeneous, Singular Case}\label{sec:opt-quad-singular}
