...
An \emph{artificial neural network} (ANN) is an $L$-layered directed graph: layer $l = 1$ is the input ($n(1) = h$ neurons), layer $l = L$ is the output ($n(L) = k$ neurons), and layers $2, \dots, L-1$ are hidden.
Each neuron $i$ in layer $l+1$ computes
$o_i^{l+1}(W^{l+1}; s) = \sigma_i^{l+1}\bigl(\sum_{j \in N(l)} w_{ji}^l\, o_j^l(W^l; s) + w_i^l\bigr)$,
where $w_{ji}^l$ are the arc weights and $\sigma_i^l(\cdot)$ is the \emph{activation function}.
...
Regularization ($\lambda\norm{w}^2$ or $\lambda\norm{w}_1$) is standard, and the hyperparameter $\lambda > 0$ is fixed via grid search: itself an exponential problem (\Cref{sec:curse-dim}).

One practical observation matters most: global optima are \emph{not} required (and may even be harmful, overfitting).
...
\subsection{The Curse of Dimensionality}\label{sec:curse-dim}
...
\begin{theorem}[Multivariate Lipschitz lower bound]\label{thm:multivariate-lip}
...
This is the \emph{curse of dimensionality}: for $n = 3$ or $5$, a grid is feasible (and is, in fact, the standard approach to hyperparameter optimization); for $n = 100$, it is absurd.
If $f$ is analytic, clever spatial branch-and-bound (\Cref{sec:global-opt}) can find the global optimum; if $f$ is a black box (no derivatives), effective heuristics exist but offer no optimality guarantee \cite{serafino2014}.
...
Most convergence results are \emph{dimension-independent} (the iteration count doesn't explicitly depend on $n$), but the cost per iteration does: computing $f$ or $\nabla f$ takes at least $\mathcal{O}(n)$, and for many problems $\mathcal{O}(n^2)$.
...
\subsection{Neighborhoods and Convergence}\label{sec:convergence}

\begin{definition}[Convergence of a sequence]\label{def:convergence}
...
\subsection{Gradients, Jacobians, and Hessians}\label{sec:grad-hess}
...
\begin{definition}[Directional derivative]\label{def:dir-deriv}
	For $f \colon \R^n \to \R$, the \emph{directional derivative} at $x$ along $d \in \R^n$ is
...
\begin{definition}[Gradient]\label{def:gradient}
	The \emph{gradient} of $f \colon \R^n \to \R$ at $x$ is the vector of partial derivatives:
...
\begin{example}[Gradient of a quadratic]
...
\begin{definition}[Differentiability]\label{def:differentiable}
	A function $f$ is \emph{differentiable} at $x$ if there exists a vector $b \in \R^n$ such that
...
\begin{theorem}[Continuity and differentiability]\label{thm:cont-diff}
...
\begin{definition}[Jacobian]\label{def:jacobian}
	For a vector-valued function $f \colon \R^n \to \R^m$, the \emph{Jacobian} at $x$ is the $m \times n$ matrix of first partial derivatives:
...
\begin{definition}[Hessian]\label{def:hessian}
	For $f \colon \R^n \to \R$ with $f \in C^2$, the \emph{Hessian} is the $n \times n$ matrix of second partial derivatives:
...
\subsection{Optimality Conditions}\label{sec:optimality-conditions}
...
\begin{theorem}[Optimality conditions]\label{thm:optimality}
...
Second, when $f$ is ``simple'' (quadratic), $\nabla f(x) = 0$ becomes a \emph{linear} system $Qx + q = 0$: quadratic models are going to be useful as building blocks.
...
\begin{definition}[Convex function]\label{def:convex}
	A function $f$ is \emph{convex} if, for all $x, z \in \R^n$ and $\alpha \in [0,1]$,
...
Sometimes the best way to prove a function is convex is \emph{by construction}: build it from known convex pieces using operations that preserve convexity.
...
\subsection{Quasi-convexity}\label{sec:quasiconvex}

In one dimension, unimodality sufficed for local methods to work.
The multivariate generalization is quasi-convexity.

\begin{definition}[Quasi-convex function]\label{def:quasiconvex}
	A function $f$ is \emph{quasi-convex} if for all $x, z$ and $\alpha \in [0,1]$,
...
\subsection{Strong Convexity and Smoothness}\label{sec:strong-convex}
...
\begin{definition}[Strong convexity]\label{def:strong-convex}
	A function $f$ is \emph{$\tau$-strongly convex} (with modulus $\tau > 0$) if $f(x) - \frac{\tau}{2}\norm{x}^2$ is convex.
...
\begin{definition}[$L$-smoothness]\label{def:L-smooth}
	A differentiable function $f$ is \emph{$L$-smooth} if its gradient is Lipschitz continuous:
...
\begin{theorem}[Joint characterization]\label{thm:hessian-bounds}
	...
	\end{equation}
	The ratio $\kappa = L / \tau$ is the \emph{condition number} of the problem.
