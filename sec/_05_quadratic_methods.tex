...
\begin{definition}[Iterative procedure]\label{def:iterative}
...
But any \emph{monotone} sequence has a limit (possibly $\pm\infty$): monotone algorithms are well-behaved in this sense.
In practice, we produce a \emph{minimizing sequence} $\{x^k\}$ such that $f(x^k) \to f^*$.
...
\subsection{Convergence for \texorpdfstring{$Q \succ 0$}{Q ≻ 0}}\label{sec:convergence-sd}

To measure progress, we use the \emph{$Q$-norm error}:
\begin{equation}\label{eq:q-norm-error}
	A(x) = \frac{1}{2}(x - x_*)^T Q (x - x_*),
\end{equation}
which equals $f(x) - f(x_*)$ (the absolute gap).

\begin{theorem}[Contraction of steepest descent]\label{thm:grad-conv}
...
\begin{definition}[Condition number]\label{def:kappa}
...
\begin{theorem}[Kantorovich inequality]\label{thm:kantorovich}
...
\subsection{Complexity and Convergence Rates}\label{sec:complexity-rates}
...
\begin{definition}[Rate of convergence]\label{def:rate-convergence}
...
\subsection{The Stopping Criterion}\label{sec:stopping}
...
\subsection{Hidden Dependence on Dimension and the \texorpdfstring{$\lambda_n = 0$}{λₙ = 0} Case}\label{sec:lambda-n-zero}
...
\begin{theorem}[Sublinear convergence for $\lambda_n = 0$]\label{thm:sublinear-quad}
...
\subsection{Conjugate Gradient Method}\label{sec:conjugate-gradient}

Steepest descent zig-zags because consecutive gradients are orthogonal but not \emph{globally} independent: the method revisits directions it has already explored.
...
\begin{theorem}[Square root decomposition]\label{thm:sqrt-decomp}
...
Translating back to $x$-space: we need directions that are orthogonal \emph{after} the $R$-transformation.

\begin{definition}[$Q$-conjugate directions]\label{def:q-conjugate}
...
\begin{theorem}[Finite termination]\label{thm:cg-finite}
...
\begin{definition}[Fletcher--Reeves formula]\label{def:fletcher-reeves}
...
\begin{theorem}[Convergence of conjugate gradient]\label{thm:cg-conv}
...
\begin{theorem}[Eigenvalue-dependent bound]\label{thm:cg-eigen-conv}
	After $k$ steps,
...
\begin{definition}[Preconditioned system]\label{def:preconditioner}
...
\subsection{Cholesky Factorization}\label{sec:cholesky}
...
\begin{definition}[Cholesky decomposition]\label{def:cholesky}
...
\begin{equation}\label{eq:cholesky-solve}
	L w = -q, \qquad L^T x = w.
\end{equation}
Each triangular solve (\emph{backsolve}) costs $\mathcal{O}(n^2)$: at step $k$, compute $x_k = (b_k - \sum_{i < k} L_{ki} x_i) / L_{kk}$.
