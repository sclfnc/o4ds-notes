...
The unifying view: each algorithm constructs a \emph{model} of $f$ (first-order, second-order, or data-driven) and uses the model to choose the next iterate.
The model is always wrong to some extent, and managing that mismatch is the core challenge.


\subsection{The Gradient Method}\label{sec:gradient-general}
...
\subsection{Fixed Stepsize}\label{sec:fixed-stepsize}
...
The $\mathcal{O}(1/\varepsilon)$ bound is not optimal for the class of $L$-smooth convex functions: $\mathcal{O}(1/\sqrt{\varepsilon})$ is achievable (see \Cref{sec:accelerated}).
...
\subsection{Inexact Line Search}\label{sec:line-search}
...
\begin{equation}\label{eq:armijo}
	...
	\qquad 0 < m_1 \ll 1.
\end{equation}
This ensures \emph{sufficient decrease}: the function drops by at least a fraction $m_1$ of what the linear model predicts.
...
The \emph{strong Wolfe condition} $|\varphi'(\alpha)| \leq m_3\, |\varphi'(0)|$ additionally prevents $\varphi'(\alpha) \gg 0$, filtering out some local maxima.

\begin{theorem}[Existence of Armijo--Wolfe stepsizes]\label{thm:aw-exist}
...
\subsection{Efficiency for General Functions}\label{sec:efficiency-general}

\begin{theorem}[Local convergence rate]\label{thm:grad-general}
...
\subsection{Twisted Gradient Methods and General Descent}\label{sec:twisted-descent}
...
\begin{definition}[Descent direction]\label{def:descent-dir}
	A direction $d^i$ is a \emph{descent direction} if $\cos(\theta^i) = \inner{d^i, \nabla f(x^i)} / (\norm{d^i} \norm{\nabla f(x^i)}) < 0$.
\end{definition}
...
\subsection{Newton's Method}\label{sec:newton-multivariate}
...
\begin{definition}[Newton direction]\label{def:newton-dir}
	If $\nabla^2 f(x^i) \succ 0$, the \emph{Newton direction} is
	$d^i = -[\nabla^2 f(x^i)]^{-1} \nabla f(x^i)$.
...
\begin{theorem}[Quadratic convergence]\label{thm:newton-quad}
	If $f \in C^3$, $\nabla f(x_*) = 0$, $\nabla^2 f(x_*) \succ 0$, then $\exists\, \delta > 0$ such that starting in $\mathcal{B}(x_*, \delta)$ gives quadratic convergence.
\end{theorem}

\begin{theorem}[Global convergence with line search]\label{thm:newton-global}
...
\subsection{Trust Region Methods}\label{sec:trust-region}
...
\begin{definition}[Trust region subproblem]\label{def:trust-region}
...
\subsection{Quasi-Newton Methods}\label{sec:quasi-newton}
...
\begin{definition}[Secant equation]\label{def:secant}
	Define $s^i = x^{i+1} - x^i$ and $y^i = \nabla f(x^{i+1}) - \nabla f(x^i)$.
	The \emph{secant equation} requires $H^{i+1} s^i = y^i$.
	For compatibility with $H^{i+1} \succ 0$, the \emph{curvature condition} $\inner{s^i, y^i} > 0$ must hold, enforceable via Armijo--Wolfe line search.
\end{definition}
...
DFP and BFGS are the endpoints of the \emph{Broyden family}: $H^{i+1} = \beta\, H^{i+1}_{\mathrm{DFP}} + (1 - \beta)\, H^{i+1}_{\mathrm{BFGS}}$ for $\beta \in [0, 1]$.
...
\subsection{Deflected Gradient Methods}\label{sec:deflected}

Twisting ($d^i = H^i(-\nabla f(x^i))$) costs at least $\mathcal{O}(n^2)$.
\emph{Deflection} is cheaper, $\mathcal{O}(n)$ by design:
...
\subsection{Nonlinear Conjugate Gradient}\label{sec:nonlinear-cg}

The conjugate gradient idea from \Cref{sec:conjugate-gradient} extends to general functions via deflection.
...
These ``$\max\{0, \cdot\}$'' modifications reset the direction to steepest descent when deflection goes wrong, a form of \emph{restart}.
...
\subsection{Heavy Ball Method}\label{sec:heavy-ball}

The heavy ball method adds \emph{momentum}: a fraction of the previous step.
...
\begin{definition}[Gelfand's formula and quasi-linear convergence]\label{def:gelfand}
	...
	This is called \emph{quasi-linear convergence}.
\end{definition}
...
\subsection{Accelerated Gradient Method}\label{sec:accelerated}

Nesterov's accelerated gradient achieves \emph{optimal} worst-case rates among first-order methods.
...
The method evaluates $\nabla f$ at an \emph{extrapolated} point $y$ rather than $x$: it ``looks ahead'' before stepping.
